...
const SettingsPage = () => {
    ...
    const { data: settings, isPending } = api.admin.account.getAccountDetails.useQuery({
        id: user?.id || 0
    }, {
        enabled: !!user?.id
    })
    ...
    const { mutate: updateEmployeeID, isPending: updateEmployeeIDIsPending } = api.admin.account.updateEmployeeID.useMutation({
        onSuccess: () => onSuccess({ description: "Employee ID updated successfully." })
    })

    const { mutate: updateSuperAdminPass, isPending: updateSuperAdminPassIsPending } = api.admin.account.updateAdminPass.useMutation({
        onSuccess: () => {
