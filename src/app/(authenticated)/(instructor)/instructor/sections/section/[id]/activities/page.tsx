...
const Attendance = () => {
  ...
  const {
    ...
    refetch,
  } = api.instructor.section.getActivities.useQuery(
    {
      sectionId: Number(id),
    },
    {
    ...
    <div className="flex flex-col gap-5 py-2">
      <div className="flex flex-row items-end justify-between gap-5">
        ...
        <div className="flex flex-row gap-2">
          <Select
            onValueChange={(e) => setActivityType(e as ActivityTypeTypes)}
          ...
          >
            ...
            <SelectContent className="w-[250px]">
              {activityTypes?.map((course) => (
                <SelectItem key={course.value} value={course.value}>
                  <span className="text-nowrap text-start">{course.label}</span>
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
          ...
          {filteredActivities
            ...
            .map((activity) => (
              <TableRow key={activity.id}>
                ...
                <TableCell className="">{activity.title}</TableCell>
                <TableCell className="">
                  {
                    activityTypes.find(
                      (at) => at.value === activity.activity_type,
                    )?.label
                  }
                </TableCell>
