...
export function AttendanceTable({
  ...
  const { mutateAsync: onCreateStudentAttendance } =
    api.instructor.section.onCreateAttendanceForStudent.useMutation({
      onSuccess: (data, a) => {
    ...
    <Table>
      <TableHeader>
        <TableRow>
          ...
          <TableHead className="w-[100px] text-center">Is Present</TableHead>
        </TableRow>
      </TableHeader>
      <TableBody>
        {studentAttendance.map((student) => (
          <TableRow key={student.studentID}>
            ...
            <TableCell className="w-[150px]">{student.studentID}</TableCell>
            <TableCell>{student.name}</TableCell>
            <TableCell className="flex w-[100px] items-center justify-center text-center">
              {
                <Checkbox
                  disabled={loadingIds.includes(student.id.studentBatchId)}
                  checked={student.isPresent}
                  onCheckedChange={async (e) => {
                    setLoadingIds((prev) => [
                      ...prev,
                      student.id.studentBatchId,
                    ]);
                    setStudents(() => {
                            ...
                            AttedanceScore: [
                              {
                                ...
                                redeemed:
                                  stud.AttedanceScore[0]?.redeemed || false,
                                attendanceId:
                                  stud.AttedanceScore[0]?.attendanceId || 0,
                                studentBatchId:
                                  stud.AttedanceScore[0]?.studentBatchId || 0,
                              },
                            ],
                          };
                        } else {
                    ...
                    await onCreateStudentAttendance({
                      ...student.id,
                      present: e as boolean,
                    });
                  }}
                />
              }
